...
describe('Padrão State: ListaRancho', () => {
  ...
  it('deve permitir adicionar item no estado Rascunho', () => {
    const lista = new ListaRancho();
    lista.adicionarItem({ codigo_item: 'ALIM-001', quantidade: 10, exige_validade: false });
    expect(lista.data.itens.length).toBe(1);
    expect(lista.data.itens[0].quantidade).toBe(10);
  });

  it('deve bloquear adicionar item sem validade quando exige_validade for true', () => {
    const lista = new ListaRancho();
    expect(() => {
      lista.adicionarItem({ codigo_item: 'ALIM-001', quantidade: 10, exige_validade: true });
    }).toThrow(ValidacaoItemError);
  });
  ...
  it('deve gerar o Payload RPA exato ao Exportar e transitar para estado final', () => {
    const lista = new ListaRancho();
    lista.definirOrigemDestino(1, 'DEP-01', 2, 'ESC-01');
    lista.data.itens.push({ codigo_item: 'ALIM-001', quantidade: 10, exige_validade: true, data_validade: '2026-10-10' });
    lista.setState(new ConsolidadaState(lista));
    ...
    expect(payload.itens[0].validade).toBe('2026-10-10');
  });
});
