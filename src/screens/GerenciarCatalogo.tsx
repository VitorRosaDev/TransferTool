  ...
  const renderStep3 = () => (
    <View style={styles.content}>
      <View style={styles.headerRow}>
        ...
        <Text style={[styles.stepTitle, { color: colors.text }]}>Executar {action}</Text>
      </View>
      <Text style={[styles.subtitle, { color: colors.textMuted }]}>
        {action?.toUpperCase()} {category?.toUpperCase()}
      </Text>
      
      {/* Placeholder para os fluxos específicos */}
      <View style={[styles.placeholder, { borderColor: colors.border }]}>
        <Ionicons name="construct-outline" size={48} color={colors.textMuted} />
        <Text style={{ color: colors.textMuted, marginTop: 12 }}>
          Interface de {action} {category} em breve...
        </Text>
      </View>
      ...
      <TouchableOpacity 
        ...
        onPress={reset}
      >
        <Text style={{ color: colors.primary, fontWeight: 'bold' }}>Voltar ao Início</Text>
      </TouchableOpacity>
    </View>
  );
